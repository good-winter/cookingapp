...
**Phase 0 与 Phase 1 已完成。种子数据已就绪，前端可以开始联调。**

## 契约 v2 已定稿

评审意见 10 条全部处置，每条下方都有「处置」标注；接口数由 11 增至 13
（新增两个删除端点）。补充的约定：排序键、上传限制、枚举下发边界、
游标失效规则、偏好保存的中间态、options 缓存策略。

## Phase 0：种子数据（阻塞点已解除）

- 12 条菜谱，含热量与碳水/蛋白/脂肪比例
- 3 个用户（含 `timezone`）+ 3 个 dev token
- 3 条帖子 + 9 个话题标签 + 1 条关注关系（u_1 关注 u_2，关注流可直接验证）
- 选项字典三张：dietModes 2 / crowds 5 / avoidFoods 5

## Phase 1：四个接口已实现

`GET /me`、`PUT /me/preferences`、`GET /preferences/options`、`GET /recipes/recommend`

启动方式见 `server/README.md`。

## 实测结论

把 u_1 的偏好改为 `vegetarian` + 忌口 `seafood` 后，推荐结果从 12 条降到 5 条，
7 条荤菜全部被剔除 —— 「偏好入库、推荐服务端算」这条决策已实际生效。

## 两点需要你知悉

1. **菜谱营养数值是演示用估值**，不是营养学准确数据，上线前需替换为可靠来源。

2. **开发机环境有两处变化**，可能影响你：
   - Flutter 已由 3.41.4 升到 **3.47.2**（与本项目 `.metadata` 记录的 revision 一致）
   - 排查发现开发机把 `PUB_HOSTED_URL` 指向了国内镜像，而该镜像的包版本滞后于
     pub.dev，这正是 `pubspec.lock` 两边不一致的根因（实测差异 85 行 vs 4 行）。
     已改为直连 pub.dev。**建议你也确认一下本机的 pub 源**，否则 lock 会继续抖动。

## 剩余

Phase 2（识别 + 统计）、Phase 3（社区）。
